...
--
-- Captured from a live WoW: Forever client (1.60.1.69913) by performing every
-- emote with and without a target and recording what the server printed back.
-- Forever has reworded a number of emotes and made some untargetable, so data
-- taken from retail does not match. The result text is server-generated and is
-- not available from the client, which is why it has to live here; it stays
-- English on non-enUS clients.
--
-- A few emotes the server accepts are absent from the client's EMOTE*_TOKEN
-- list, so they cannot be captured and were verified by hand instead.
